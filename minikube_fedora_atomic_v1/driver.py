...
from minikube_fedora_atomic_v1 import template_def
...
class Driver(driver.KubernetesDriver):
    ...
    def provides(self):
        return [
            {'server_type': 'vm',
             'os': 'fedora-atomic-minikube',
             'coe': 'kubernetes'},
        ]

    def get_template_definition(self):
        LOG.info("########## get_template_definition")
        return template_def.AtomicMinikubeTemplateDefinition()
    ...
    def upgrade_cluster(self, context, cluster, cluster_template,
        ...
        # kube_tag changed
        heat_params["minikube_version"] = cluster_template.labels["minikube_version"]
        heat_params["kubectl_version"] = cluster_template.labels["kubectl_version"]
        heat_params["etcd_version"] = cluster_template.labels["etcd_version"]
