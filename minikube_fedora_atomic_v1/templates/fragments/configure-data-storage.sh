...
if [ -n "$DATA_VOLUME_SIZE" ] && [ "$DATA_VOLUME_SIZE" -gt 0 ]; then
    if [ "$ENABLE_CINDER" == "False" ]; then
        ...
        while [ ${attempts} -gt 0 ]; do
            device_name=$($ssh_cmd ls /dev/disk/by-id | grep ${DATA_VOLUME:0:20}$)
        ...
        echo "${device_path} /data xfs defaults 0 0" >> /etc/fstab
        $ssh_cmd mount -a
        $ssh_cmd chown -R root.root /data
        $ssh_cmd chmod 755 /data
