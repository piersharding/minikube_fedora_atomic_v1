...
ADMIN_RBAC=/etc/kubernetes/kubernetes-admin-rbac.yaml
...
# if [ -n "${TRUST_ID}" ] && [ "$(echo "${CLOUD_PROVIDER_ENABLED}" | tr '[:upper:]' '[:lower:]')" = "true" ]; then
    occm_image="${OCCM_CONTAINER_INFRA_PREFIX:-docker.io/k8scloudprovider/}openstack-cloud-controller-manager:${CLOUD_PROVIDER_TAG}"
    OCCM=/etc/kubernetes/openstack-cloud-controller-manager.yaml
...
- apiVersion: rbac.authorization.k8s.io/v1
  ...
  - kind: ServiceAccount
    ...
    namespace: kube-system
# add crb for shared-informers because needs access to nodes and services
- apiVersion: rbac.authorization.k8s.io/v1
  kind: ClusterRoleBinding
  metadata:
    name: system:cloud-node-controller-shared-informers
  roleRef:
    apiGroup: rbac.authorization.k8s.io
    kind: ClusterRole
    name: system:cloud-controller-manager
...
# deploy the Cinder standalone provisioner
