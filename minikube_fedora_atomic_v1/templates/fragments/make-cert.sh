...
sans="${sans},DNS:minikubeCA,DNS:kubernetes,DNS:kubernetes.default,DNS:kubernetes.default.svc,DNS:kubernetes.default.svc.cluster.local,DNS:localhost,IP:10.96.0.1,IP:10.0.0.1"
...
echo -e "$CA_KEY" > $cert_dir/ca-pkcs8.key
chmod 400 $cert_dir/ca-pkcs8.key
...
#
# #API Server Client
# cat > ${cert_dir}/apiserver-etcd-client.conf <<EOF
# [req]
# distinguished_name = req_distinguished_name
# req_extensions     = req_ext
# prompt = no
...
# generate_certificates apiserver ${cert_dir}/apiserver.conf
# generate_certificates apiserver-etcd-client-magnum ${cert_dir}/apiserver-etcd-client.conf
# generate_certificates apiserver-kubelet-client-magnum ${cert_dir}/apiserver-kubelet-client.conf
# generate_certificates front-proxy-client-magnum ${cert_dir}/front-proxy-client.conf
# generate_certificates peer-magnum ${cert_dir}/peer.conf
# cp ${cert_dir}/ca.crt ${cert_dir}/front-proxy-ca.crt
# cp ${cert_dir}/ca.key ${cert_dir}/front-proxy-ca.key
# chmod 400 ${cert_dir}/front-proxy-ca.key
...
# echo -e "${KUBE_SERVICE_ACCOUNT_KEY}" > ${cert_dir}/sa.pub
# echo -e "${KUBE_SERVICE_ACCOUNT_PRIVATE_KEY}" > ${cert_dir}/sa.key
...
# $ssh_cmd cp ${cert_dir}/*.crt ${cert_dir}/*.key /etc/etcd/certs
# $ssh_cmd mkdir -p ${cert_dir}/etcd
# $ssh_cmd cp ${cert_dir}/*.crt ${cert_dir}/*.key ${cert_dir}/etcd
